...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/automations", async (req, res) => {
    try {
      ...
      const { calculateWorkflowComplexity, countWorkflowConditions } =
        await import("./utils/complexityCalculator");
      ...
      try {
        ...
        const hasAiService =
          validatedData.actions?.some((action: any) =>
            ["openai", "anthropic", "perplexity", "ollama"].includes(
              action.appId,
            ),
          ) ||
          ["openai", "anthropic", "perplexity", "ollama"].includes(
            validatedData.actionAppId,
          );
      ...
      if (error instanceof z.ZodError) {
        return res
          .status(400)
          .json({ message: "Invalid automation data", errors: error.errors });
      }
  ...
  app.patch("/api/automations/:id", async (req, res) => {
    try {
      ...
      if (
        ...
        try {
          ...
          if (existingAutomation) {
            ...
            // Check for AI Integrator achievement
            const hasAiService =
              mergedData.actions?.some((action: any) =>
                ["openai", "anthropic", "perplexity", "ollama"].includes(
                  action.appId,
                ),
              ) ||
              ["openai", "anthropic", "perplexity", "ollama"].includes(
                mergedData.actionAppId,
              );
      ...
      const updatedAutomation = await storage.updateAutomation(
        id,
        validatedData,
      );
  ...
  app.get(
    "/api/execution-history/automation/:automationId",
    async (req, res) => {
      try {
        ...
        const histories =
          await storage.getExecutionHistoriesByAutomationId(automationId);
        res.json(histories);
      } catch (error) {
        res
          .status(500)
          .json({ message: "Failed to fetch execution histories" });
      }
    },
  );
    ...
    try {
      ...
      const enrichedHistories = histories.map((history) => {
        const automation = automations.find(
          (a) => a.id === history.automationId,
        );
        return {
          ...history,
          automationName:
            automation?.name || `Automation ${history.automationId}`,
  ...
  app.get("/api/execution-history/:automationId", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({
          message: "Failed to fetch execution histories for automation",
        });
    }
  });
    ...
    try {
      ...
      if (error instanceof z.ZodError) {
        return res
          .status(400)
          .json({
            message: "Invalid execution history data",
            errors: error.errors,
          });
      }
    ...
    try {
      ...
      switch (templateId) {
        case "linkedin-ai-post":
          ...
          description =
            "Automatically generate and post content to LinkedIn on a schedule";
        ...
        case "mailchimp-newsletter":
          ...
          description =
            "Create and send personalized newsletters through Mailchimp";
        ...
        case "ai-customer-service":
          ...
          actionConfig = {
            model: "gpt-4o",
            toneStyle: "helpful",
            responseTemplate: "customer-service",
          };
          break;
      ...
      res
        .status(500)
        .json({ message: "Failed to create automation from template" });
    }
  });
  ...
  app.get("/api/achievements/automation/:automationId", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: "Failed to fetch unlocked achievements" });
    }
  });
  ...
  app.post("/api/automations/:id/run", async (req, res) => {
    try {
      ...
      if (!automation.active) {
        return res
          .status(400)
          .json({ message: "Cannot run inactive automation" });
      }
  ...
  app.get("/api/auth/:service/callback", async (req, res) => {
    ...
    console.log(
      `Redirecting from old callback format to new format for ${service}`,
    );
  ...
  app.get("/api/callback/:service", async (req, res) => {
    ...
    if (service === "instagram") {
      ...
      console.error(
        `Invalid state parameter for ${service} OAuth. Got state: ${state}`,
      );
      ...
      if (isValidOAuthService(service)) {
        ...
        if (service === "linkedin") {
          ...
          tokenResponse = await axios
            .post(config.tokenURL, formData.toString(), {
              headers: {
                "Content-Type": "application/x-www-form-urlencoded",
                Accept: "application/json",
              },
  ...
  app.get("/api/connections", (req, res) => {
    ...
    storage
      .getAppConnectionsByUser(req.user.id)
      .then((connections) => {
  ...
  app.post("/api/direct-token-connect/linkedin", async (req, res) => {
    try {
      ...
      if (!req.isAuthenticated()) {
        return res
          .status(401)
          .json({ error: "You must be logged in to connect LinkedIn" });
      }
      ...
      try {
        ...
        if (!userResponse.ok) {
          throw new Error(
            `LinkedIn API error: ${userResponse.status} ${userResponse.statusText}`,
          );
        }
        ...
        const savedConnection = await storage.saveAppConnection({
          appId: "linkedin",
          userId: req.user.id,
          username: username.trim(),
          credentials: {
  ...
  app.get("/api/auth/:service/simulated-callback", (req, res) => {
    ...
    if (service === "instagram") {
      console.log(
        "Bypassing strict state verification for Instagram OAuth in simulated callback",
      );
    } else if (!state || !verifyOAuthState(req, service, state as string)) {
      console.error(
        `Invalid state parameter for ${service} OAuth simulated callback. Got state: ${state}`,
      );
      return res.status(400).json({ error: "Invalid state parameter" });
    }
  ...
  app.get("/api/app-connections", async (req, res) => {
    try {
      if (!req.isAuthenticated()) {
        return res
          .status(401)
          .json({ message: "You must be logged in to view connections" });
      }
      
      console.log(`Fetching app connections for user ${req.user.id}...`);
      const connections = await storage.getAppConnectionsByUser(req.user.id);
      console.log(`Found ${connections.length} connections:`, connections.map(c => ({
        id: c.id,
        appId: c.appId,
        username: c.username,
  ...
  app.post("/api/facebook-auth", async (req, res) => {
    try {
      ...
      if (!req.isAuthenticated()) {
        return res
          .status(401)
          .json({ error: "You must be logged in to connect apps" });
      }
      ...
      if (!accessToken || !userID) {
        return res
          .status(400)
          .json({ error: "Missing required Facebook auth parameters" });
      }
      ...
      if (mockData) {
        ...
        try {
          ...
          req.session.oauthCredentials[
            serviceType === "instagram" ? "instagram" : "facebook-ads"
          ] = credentials;
      ...
      if (serviceType === "instagram") {
        ...
        try {
          ...
          if (!fbResponse.ok) {
            throw new Error(
              `Failed to fetch Instagram accounts: ${fbResponse.statusText}`,
            );
          }
          ...
          if (!igAccountResponse.ok) {
            throw new Error(
              `Failed to fetch Instagram business account: ${igAccountResponse.statusText}`,
            );
          }
          ...
          if (!igAccountData.instagram_business_account) {
            return res.status(400).json({
              error:
                "No Instagram business account found. Please connect your Instagram account to your Facebook page.",
            });
          }
          ...
          if (!igProfileResponse.ok) {
            throw new Error(
              `Failed to fetch Instagram profile: ${igProfileResponse.statusText}`,
            );
          }
        ...
        try {
          ...
          if (!fbProfileResponse.ok) {
            throw new Error(
              `Failed to fetch Facebook profile: ${fbProfileResponse.statusText}`,
            );
          }
  ...
  app.post("/api/settings/api-keys", async (req, res) => {
    try {
      ...
      if (
        newKeys.anthropic &&
        newKeys.anthropic !== req.session.apiKeys.anthropic
      ) {
        try {
      ...
      if (
        newKeys.perplexity &&
        newKeys.perplexity !== req.session.apiKeys.perplexity
      ) {
        try {
          ...
          if (!response.ok) {
            ...
            return res.status(400).json({
              message:
                "Invalid Perplexity API key. Please check your key and try again.",
              service: "perplexity",
            });
          }
    ...
    try {
      ...
      switch (appId) {
        case "instagram":
          ...
          console.log(
            "Using Instagram Graph API Client ID with direct Instagram OAuth:",
            instagramClientId,
          );
        ...
        default:
          ...
          return res.status(400).json({
            ...
            message:
              "This service needs to be integrated manually with proper OAuth credentials.",
          });
      }
  ...
  app.get("/api/simulated-login", async (req, res) => {
    try {
      ...
      if (!req.isAuthenticated()) {
        return res.redirect(
          "/auth?redirect=" + encodeURIComponent("/app-connections"),
        );
      }
      ...
      const existingConnection = await storage.getAppConnectionByUserAndApp(
        req.user.id,
        service,
      );
  ...
  app.post("/api/app-connections/:appId/connect", async (req, res) => {
    try {
      if (!req.isAuthenticated()) {
        return res
          .status(401)
          .json({ message: "You must be logged in to connect apps" });
      }
      ...
      if (existingConnection) {
        // Update existing connection
        const updatedConnection = await storage.updateAppConnection(
          existingConnection.id,
          {
            username,
            permissions,
            credentials: {
              token: apiIntegration ? "api-key-integration" : "mock-token",
            },
          },
        );
    ...
    try {
      ...
      if (oauthError) {
        ...
        return renderErrorPage(
          res,
          service,
          new Error(`Authentication error: ${oauthError}`),
        );
      }
      ...
      if (!code) {
        ...
        return renderErrorPage(
          res,
          service,
          new Error("No authentication code received"),
        );
      }
        ...
        try {
          ...
          switch (service) {
            case "instagram":
              ...
              console.log(
                "Using Instagram Graph API credentials - Client ID:",
                clientId,
              );
            ...
            case "linkedin":
              ...
              clientSecret =
                process.env.LINKEDIN_CLIENT_SECRET || "YOUR_APP_SECRET";
            ...
            case "google-calendar":
              ...
              clientSecret =
                process.env.GOOGLE_CLIENT_SECRET || "YOUR_APP_SECRET";
            ...
            case "slack":
              ...
              clientSecret =
                process.env.SLACK_CLIENT_SECRET || "YOUR_APP_SECRET";
            ...
            case "twitter":
              ...
              clientSecret =
                process.env.TWITTER_CLIENT_SECRET || "YOUR_APP_SECRET";
            ...
            case "notion":
              ...
              clientSecret =
                process.env.NOTION_CLIENT_SECRET || "YOUR_APP_SECRET";
            ...
            case "trello":
              ...
              clientSecret =
                process.env.TRELLO_CLIENT_SECRET || "YOUR_APP_SECRET";
            ...
            case "microsoft":
              ...
              clientSecret =
                process.env.MICROSOFT_CLIENT_SECRET || "YOUR_APP_SECRET";
            ...
            default:
              ...
              console.log(
                `Using mock token for ${service} as no token exchange implementation exists`,
              );
              break;
          }
            ...
            try {
              ...
              if (!tokenResponse.ok) {
                ...
                try {
                  ...
                  if (service === "instagram") {
                    console.error(
                      `Instagram token exchange error. Full response:`,
                      {
                        status: tokenResponse.status,
                        statusText: tokenResponse.statusText,
                        body: errorBody,
              ...
              try {
                ...
                throw new Error(
                  `Invalid token response format: ${parseError.message}`,
                );
              }
              ...
              if (
                ...
                try {
                  ...
                  console.log(
                    "Instagram Graph API long-lived token exchange URL:",
                    longLivedTokenUrl,
                  );
              ...
              if (accessToken) {
                ...
                switch (service) {
                  case "instagram":
                    ...
                    userEndpoint =
                      "https://graph.facebook.com/v16.0/me/accounts?fields=instagram_business_account{username,profile_picture_url,id,name}&access_token=" +
                      accessToken;
                    console.log(
                      "Using Facebook Graph API for Instagram user data:",
                      userEndpoint,
                    );
                ...
                if (userEndpoint) {
                  ...
                  if (userResponse.ok) {
                    ...
                    if (userData) {
                      if (service === "instagram") {
                        ...
                        if (
                          ...
                          console.log(
                            "Instagram data structure unexpected:",
                            userData,
                          );
                        ...
                        service === "linkedin" &&
                        userData.localizedFirstName
                      ) {
                        username = `${userData.localizedFirstName} ${userData.localizedLastName || ""}`;
                      } else if (
                        (service.startsWith("google") || service === "gmail") &&
                        userData.email
                      ) {
                    ...
                    console.error(
                      `Failed to get user profile: ${userResponse.status} ${userResponse.statusText}`,
                    );
                  }
                }
              }
  ...
  app.delete("/api/app-connections/:appId", async (req, res) => {
    try {
      ...
      if (!req.isAuthenticated()) {
        return res
          .status(401)
          .json({ message: "You must be logged in to disconnect apps" });
      }
      ...
      const connection = await storage.getAppConnectionByUserAndApp(
        req.user.id,
        appId,
      );
  ...
  app.post("/api/services/openai/search", async (req, res) => {
    try {
      const { query, searchRecency, temperature, maxTokens, systemMessage } =
        req.body;
      if (!query) {
        return res.status(400).json({ message: "Query is required" });
      }

      const result = await openaiService.performSearch(
        query,
        {
          searchRecency,
          temperature,
  ...
  app.post("/api/services/openai/analyze-topic", async (req, res) => {
    try {
      const { topic, searchRecency, temperature, systemMessage, maxTokens } =
        req.body;
      if (!topic) {
        return res.status(400).json({ message: "Topic is required" });
      }

      const result = await openaiService.analyzeTopic(
        topic,
        {
          searchRecency,
          temperature,
  ...
  app.post("/api/services/openai/research-question", async (req, res) => {
    try {
      const { question, searchRecency, temperature, systemMessage, maxTokens } =
        req.body;
      if (!question) {
        return res.status(400).json({ message: "Question is required" });
      }

      const result = await openaiService.researchQuestion(
        question,
        {
          searchRecency,
          temperature,
  ...
  app.post("/api/services/anthropic/rewrite-content", async (req, res) => {
    try {
      ...
      if (!text || !instructions) {
        return res
          .status(400)
          .json({ message: "Text and instructions are required" });
      }
  ...
  app.post("/api/services/ollama/analyze-text", async (req, res) => {
    try {
      ...
      const result = await ollamaService.analyzeText(
        text,
        task,
        model || "llama3",
      );
  ...
  app.post("/api/services/text-processor/format", async (req, res) => {
    try {
      ...
      const result = await textProcessorService.formatText(text, targetFormat, {
        includeHeadings,
        includeLists,
        indentationSpaces,
      });
    ...
    try {
      ...
      if (automation.complexity < 2) {
        healthReport.improvementSuggestions.push({
          category: "complexity",
          message:
            "Add conditional steps to make your workflow more sophisticated",
          priority: "medium",
        });
      }
      ...
      if (
        ...
        Object.keys(automation.triggerConfig).length < 2
      ) {
        healthReport.improvementSuggestions.push({
          category: "configuration",
          message:
            "Add more specific trigger conditions to reduce false positives",
          priority: "low",
        });
      }
  ...
  app.patch("/api/automations/:id/health-score", async (req, res) => {
    try {
      ...
      if (
        ...
        return res
          .status(400)
          .json({ message: "Health score must be a number between 0 and 100" });
      }

      const automation = await storage.updateAutomationHealthScore(
        id,
        healthScore,
      );
      if (!automation) {
  ...
  app.patch("/api/automations/:id/complexity", async (req, res) => {
    try {
      ...
      if (typeof complexity !== "number" || complexity < 1) {
        return res
          .status(400)
          .json({ message: "Complexity must be a positive number" });
      }

      const automation = await storage.updateAutomationComplexity(
        id,
        complexity,
      );
      if (!automation) {
  ...
  app.get("/api/automations/:id/achievements", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: "Failed to fetch automation achievements" });
    }
  });
        ...
        suggestions = [
          {
            ...
            description:
              "Schedule posts to multiple social media platforms from a single calendar interface.",
            difficulty: "beginner",
          ...
          },
          {
            ...
            description:
              "Analyze the sentiment of incoming documents or emails and categorize them by priority.",
          ...
          },
          {
            ...
            description:
              "Consolidate messages from multiple platforms and generate AI-assisted responses.",
            difficulty: "advanced",
  ...
  app.get("/api/assistant/personalized-suggestions", async (req, res) => {
    try {
      ...
      const dbSuggestions =
        await storage.getPersonalizedWorkflowSuggestions(count);
      ...
      const suggestions =
        await workflowSuggestionService.generateWorkflowSuggestions(
          req.user.id,
          count,
        );
  ...
  app.post("/api/assistant/generate-store-suggestions", async (req, res) => {
    try {
      ...
      const suggestions =
        await workflowSuggestionService.generateAndStoreSuggestions(
          req.user.id,
          count,
        );
  ...
  app.post("/api/assistant/create-workflow", async (req, res) => {
    try {
      ...
      if (!name || !triggerAppId || !actionAppId) {
        return res
          .status(400)
          .json({ message: "Missing required workflow information" });
      }
  ...
  function getContextHelp(
    contextId: string,
  ): { text: string; context: string; suggestions: string[] } | null {
    ...
    return help
      ? { text: help.text, context: contextId, suggestions: help.suggestions }
      : null;
  }
